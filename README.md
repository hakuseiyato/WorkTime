...
- 監視対象を「起動中のアプリ」から選んで登録 (プロセス名を手入力しなくてよい)
...
設定 → 「監視対象プロセス」の **「起動中のアプリから選択…」** を押すと、今ウィンドウを開いているアプリの一覧が出る。チェックしたものがそのまま監視対象になる。

- **紐づく先はアプリ本体 (実行ファイル名)** であって、開いているファイルではない。一度選べば設定に残り、次回以降そのアプリを起動するたびに自動で計測される
- 一覧には実行ファイルの説明文 (VS Code なら "Visual Studio Code") と、実際のプロセス名がバッジで並ぶ
- すでに監視対象のアプリは最初からチェック済みで開く。チェックを外すと監視対象から外れる
- 常駐サービスと Windows のシェル基盤プロセス (ApplicationFrameHost 等) は一覧に出ない

プロセス名を手で入力する方法も残してあるが、**タスクマネージャーの表示は `Code.exe` のように拡張子付きで、そのままコピーすると一致しない**。一覧から選べばこの問題は起きない。
